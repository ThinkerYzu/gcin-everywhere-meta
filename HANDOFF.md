...
4. **Voice Phase A — live interactive test** (next session) — daemon real-mode model load on the RTX 3090 over the socket is **verified** (loads Breeze-ASR-26 on cuda:0 in ~6 s, emits `ready`); `--mock` protocol test + engine JSON-parser test pass; engine builds clean. Remaining is the interactive piece that needs a live GNOME/IBus session: enter voice with `Ctrl+Alt+0`, speak into the mic, `Enter`-commit into a real app; tune the too-short/silence threshold; confirm the 語→🎤→…→語 panel transitions live. Install: `voiced/requirements.txt` venv + `systemctl --user enable --now gcin-voiced.service` (or run `gcin-voiced.py` directly).
